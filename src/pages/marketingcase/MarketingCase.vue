    ...
    <el-main>
      ...
      >
        <el-form :model="addForm" :rules="addRules" ref="addForm" label-width="120px">
          <el-form-item label="标题：" prop="marketing_title">
            <el-input type="text" v-model.trim="addForm.marketing_title" clearable></el-input>
          </el-form-item>
          <el-form-item label="上传图片:">
            ...
            <span>
              ...
              <span class="col-redtip">大小：391 X 290 px</span>
            </span>
          </el-form-item>
          <el-form-item label="图片地址：" prop="marketing_img_url">
            <el-input type="text" v-model.trim="addForm.marketing_img_url" clearable disabled></el-input>
          </el-form-item>
          <el-form-item label="标签：">
            <vue-tags-input
              ...
              placeholder="请填写标签：（按Enter键或换行键确定）"
            />
          </el-form-item>
          <el-form-item label="案例关联外链：" prop="resource_relate_url">
            <el-input type="text" v-model.trim="addForm.resource_relate_url" clearable></el-input>
          </el-form-item>
          <el-form-item label="案例排序：" prop="order_num">
            <el-input
              type="text"
      ...
      >
        ...
        <el-table-column slot="showimg" label="图片展示" align="center" width="120">
          <template slot-scope="scope">
            <div class="tableshowimg">
              <img
                @click="openShowBigImg(scope.row.marketing_img_url)"
                :src="require('./../../server/public' + scope.row.marketing_img_url)"
                alt="表格图片"
              >
            </div>
          </template>
        </el-table-column>
        <el-table-column slot="tag" label="标签" align="center">
          <template slot-scope="scope">
            <div
              v-for="(item,index) in scope.row.marketing_tags.split(',')"
              :key="index"
              class="showtabletag"
            >
              <el-tag>{{item}}</el-tag>
        ...
        <el-table-column slot="link" label="案例关联外链" align="center">
          <template slot-scope="scope">
...
import {
  selectMarketingCase,
  getMarketingCaseList,
  addMarketingCase,
  updateMarketingCase,
  deleteMarketingCase,
  changeMarketingCaseInvaild
} from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      colConfigs: [
        ...
        { label: "标题", prop: "marketing_title", align: "center" },
        { slot: "showimg" },
        { label: "图片地址", prop: "marketing_img_url", align: "center" },
        { slot: "tag" },
        { slot: "link" },
        { label: "案例排序", prop: "order_num", width: "80", align: "center" },
        { slot: "invaild" },
  ...
  methods: {
    ...
    selectFun() {
      selectMarketingCase({ selectKey: this.selectKey }).then(res => {
        this.tableData = res;
    ...
    getRowKey(row) {
      //表格展开行才添加该属性
      return row.marketing_id; //需要数据的唯一值
    },
    //分页
    ...
    addFun() {
      this.addDialog = true;
      this.publicType = "add";
      this.publicTitle = "添加营销案例";
    },
    updateFun(ins) {
      this.publicType = "update";
      this.publicTitle = "编辑营销案例";
      this.publicId = ins.marketing_id;
      this.addForm.marketing_title = ins.marketing_title;
      this.addForm.marketing_img_url = ins.marketing_img_url;
      this.addForm.marketing_tags = "";
      let arr = ins.marketing_tags.split(",");
      let all = [];
      ...
      this.imageUrl = require("./../../server/public" + ins.marketing_img_url);
      if (ins.invaild == true) {
    ...
    deleteFun(rows) {
      ...
      that
        ...
        .then(() => {
          deleteMarketingCase({
            marketing_id: rows.marketing_id
          })
            .then(res => {
      ...
      that.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (that.tags.length == 0) {
            that.$message.info("请输入标签，按Enter键或换行键确定");
          } else {
            ...
            if (that.publicType == "add") {
              addMarketingCase({
                marketing_title: that.addForm.marketing_title,
                marketing_img_url: that.addForm.marketing_img_url,
                marketing_tags: str,
    ...
    deleteImg() {
      this.imageUrl = "";
      this.addForm.marketing_img_url = "";
      this.fileList = [];
    },
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.addForm.marketing_img_url = res.avatarUrl;
    },
